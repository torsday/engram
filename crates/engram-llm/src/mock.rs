...
fn failure_to_error(f: InjectFailure) -> Error {
    match f {
        InjectFailure::Timeout(d) => Error::Timeout {
...
mod tests {
    ...
    async fn template_mode_is_deterministic() {
        ...
        let r1 = mock
            .complete(&p, &model(), &CompleteOptions::default())
            .await
            .unwrap();
        let r2 = mock
            .complete(&p, &model(), &CompleteOptions::default())
    ...
    async fn scripted_miss_fails_loudly_with_hash() {
        ...
        assert!(
            msg.contains("no scripted response for prompt hash"),
            "got: {msg}"
        );
    }
    ...
    async fn token_accounting_math() {
        ...
        let res = mock
            .complete(&prompt(&input), &model(), &CompleteOptions::default())
            .await
            .unwrap();
    ...
    async fn streaming_chunk_shape_valid() {
        let mock = MockLlmProvider::builder()
            ...
            .build();
        let mut s = mock
            .complete_streamed(&prompt("chunk test"), &model(), &CompleteOptions::default())
            .await
            .unwrap();
    ...
    async fn failure_server_error_injected() {
        let mock = MockLlmProvider::builder()
            ...
            .build();
        let err = mock
            .complete(&prompt("x"), &model(), &CompleteOptions::default())
            .await
            .unwrap_err();
    ...
    async fn failure_consumed_then_normal_response() {
        let mock = MockLlmProvider::builder()
            ...
            .build();
        // First call: error
        mock.complete(&prompt("x"), &model(), &CompleteOptions::default())
            .await
            .unwrap_err();
        // Second call: normal
        mock.complete(&prompt("x"), &model(), &CompleteOptions::default())
    ...
    async fn partial_stream_then_disconnect() {
        ...
        let mut s = mock
            .complete_streamed(&prompt("partial"), &model(), &CompleteOptions::default())
            .await
            .unwrap();
    ...
    async fn call_count_recorded() {
        ...
        mock.complete(&prompt("a"), &model(), &CompleteOptions::default())
            .await
            .unwrap();
        mock.complete(&prompt("b"), &model(), &CompleteOptions::default())
            .await
            .unwrap();
    ...
    async fn call_record_captures_prompt() {
        let mock = MockLlmProvider::builder().mode_echo().build();
        mock.complete(&prompt("my prompt"), &model(), &CompleteOptions::default())
            .await
            .unwrap();
